...
def calc_means(bund_param_dict, homeTeam, awayTeam):
    return [np.exp(bund_param_dict['attack_'+homeTeam] + bund_param_dict['defence_'+awayTeam] + bund_param_dict['home_adv']),
            np.exp(bund_param_dict['defence_'+homeTeam] + bund_param_dict['attack_'+awayTeam])]
...
def solve_parameters(dataset, debug = False, init_vals=None, options={'disp': True, 'maxiter':100},
    ...
    def estimate_paramters(bund_params):
        score_coefs = dict(zip(teams, bund_params[:n_teams]))
        defend_coefs = dict(zip(teams, bund_params[n_teams:(2*n_teams)]))
        rho, gamma = bund_params[-2:]
...
def dixon_coles_simulate_match(bund_params_dict, homeTeam, awayTeam, max_goals=10):
    team_avgs = calc_means(bund_params_dict, homeTeam, awayTeam)
    team_pred = [[poisson.pmf(i, team_avg) for i in range(0, max_goals+1)] for team_avg in team_avgs]
    output_matrix = np.outer(np.array(team_pred[0]), np.array(team_pred[1]))
    correction_matrix = np.array([[rho_correction(home_goals, away_goals, team_avgs[0],
                                                   team_avgs[1], bund_params_dict['rho']) for away_goals in range(2)]
...
def build_temp_model(dataset, time_diff, xi=0.000, init_params=None):
    ...
    bund_params = solve_parameters_decay(train_dataset, xi=xi, init_vals=init_params)
    predictive_score = sum([np.log(get_1x2_probs(dixon_coles_simulate_match(
                    bund_params, row.HomeTeam, row.AwayTeam))[row.FTR]) for row in test_dataset.itertuples()])
    return predictive_score    
    
def check_xi(match_day):
    xi_score = build_temp_model(bund_1820, match_day, xi=my_xi)
    return xi_score

def bund_match_day(HomeTeam, AwayTeam):
    matrix = dixon_coles_simulate_match(bund_params, HomeTeam, AwayTeam, max_goals=10)
...
bund_1820 = pd.DataFrame()
for year in range(18,20):
    bund_1820 = pd.concat((bund_1820, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/D1.csv".format(year, year+1, sort=True))))
bund_1820['Date'] = pd.to_datetime(bund_1820['Date'],  format='%d/%m/%Y')
bund_1820['time_diff'] = (max(bund_1820['Date']) - bund_1820['Date']).dt.days
bund_1820 = bund_1820[['HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]
bund_1820 = bund_1820.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})
bund_1820 = bund_1820.dropna(how='all')
...
HomeTeam = ['Freiburg', 'Mainz', 'Union Berlin', 'Wolfsburg', 'Leverkusen']
AwayTeam = ['Hoffenheim', 'Bayern Munich', 'Werder Bremen', 'Dortmund',
            'Ein Frankfurt']
