...
def champ_match_day(HomeTeam, AwayTeam):
    matrix = dixon_coles_simulate_match(L1_params, HomeTeam, AwayTeam, max_goals=5)
...
L1_1820 = pd.DataFrame()
for year in range(18,21):
    L1_1820 = pd.concat((L1_1820, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/E2.csv".format(year, year+1, sort=True))))
L1_1820['Date'] = pd.to_datetime(L1_1820['Date'],  format='%d/%m/%Y')
L1_1820['time_diff'] = (max(L1_1820['Date']) - L1_1820['Date']).dt.days
L1_1820 = L1_1820[['HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]
L1_1820 = L1_1820.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})
L1_1820 = L1_1820.dropna(how='all')
     
L1_params = solve_parameters_decay(L1_1820, xi = 0.00325)

L1_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
                                      'Away win', '1X', 'X2', '12', 'BTTS', 'No BTTS'])

HomeTeam = ['Blackpool', 'Charlton', 'Crewe', 'Doncaster', 'Gillingham',
            'Ipswich', 'Lincoln', 'Milton Keynes Dons', 'Oxford',
            'Portsmouth', 'Sunderland', 'Wigan']
AwayTeam = ['Bristol Rvs', 'Hull', 'Shrewsbury', 'Peterboro', 
            'Plymouth', 'Fleetwood Town', 'AFC Wimbledon', 
            'Rochdale', 'Burton', 'Accrington', 'Northampton', 
            'Swindon']
