...
france_1820 = pd.DataFrame()
for year in range(18,21):
    france_1820 = pd.concat((france_1820, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/F1.csv".format(year, year+1, sort=True))))
france_1820['Date'] = pd.to_datetime(france_1820['Date'],  format='%d/%m/%Y')
france_1820['time_diff'] = (max(france_1820['Date']) - france_1820['Date']).dt.days
france_1820 = france_1820[['HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]
france_1820 = france_1820.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})
france_1820 = france_1820.dropna(how='all')
     
france_params = solve_parameters_decay(france_1820, xi = 0.00325)

france_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
                                      'Away win', '1X', 'X2', '12', 'BTTS', 'No BTTS'])

HomeTeam = ['St Etienne', 'Angers', 'Metz', 'Nice', 'Strasbourg',
            'Reims', 'Rennes']
AwayTeam = ['Marseille', 'Dijon', 'Nimes', 'Brest', 'Montpellier', 
            'Monaco', 'Paris SG']
