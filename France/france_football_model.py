...
for year in range(18,21):
    ...
    france_all = pd.concat((france_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/F1.csv".format(year, year+1, sort=True))))

# Ensure that the date is ina  sensible format, day/month/year
france_all['Date'] = pd.to_datetime(france_all['Date'],  format='%d/%m/%Y')
...
france_all['time_diff'] = (max(france_all['Date']) - france_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
france_1720 = france_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
france_1720 = france_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
france_1720 = france_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
france_YC_1720 = france_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
france_YC_1720 = france_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
france_YC_1720 = france_YC_1720.dropna(how='all')
...
france_params = solve_parameters_decay(france_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
france_params_YC = solve_parameters_decay_YC(france_YC_1720, xi = 0.00325)
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(france_params, i, j, max_goals=5)
    ...
    matrix_YC = dixon_coles_simulate_match(france_params_YC, i, j, max_goals=10)
    ...
    france_prediction = france_prediction.append(home_away_trans)
