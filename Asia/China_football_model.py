...
# -*- coding: utf-8 -*-
"""
Created on Thu May 25 17:01:08 2023
...
chn_all = pd.DataFrame()

    
    # Concatenate all of them together into 1 DataFrame
chn_all = pd.read_csv("https://www.football-data.co.uk/new/CHN.csv")

# Ensure that the date is ina  sensible format, day/month/year
chn_all['Date'] = pd.to_datetime(chn_all['Date'],  format='%d/%m/%Y')
...
chn_all['time_diff'] = (max(chn_all['Date']) - chn_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
chn_1720 = chn_all[['Date', 'Home','Away', 'HG','AG', 'Res', 'time_diff']]

# Rename some columns to sensible names
chn_1720 = chn_1720.rename(columns={'HG': 'HomeGoals', 'AG': 'AwayGoals'})

# Not interested in anything with na in this dataset
chn_1720 = chn_1720.dropna(how='all')
...
chn_params = solve_parameters_decay(chn_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
chn_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(chn_params, i, j, max_goals=10)
    ...
    chn_prediction = chn_prediction.append(home_away_trans)
