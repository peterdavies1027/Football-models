...
# -*- coding: utf-8 -*-
"""
Created on Thu May 25 17:03:18 2023
...
jpn_all = pd.DataFrame()

    
    # Concatenate all of them together into 1 DataFrame
jpn_all = pd.read_csv("https://www.football-data.co.uk/new/JPN.csv")

# Ensure that the date is ina  sensible format, day/month/year
jpn_all['Date'] = pd.to_datetime(jpn_all['Date'],  format='%d/%m/%Y')
...
jpn_all['time_diff'] = (max(jpn_all['Date']) - jpn_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
jpn_1720 = jpn_all[['Date', 'Home','Away', 'HG','AG', 'Res', 'time_diff']]

# Rename some columns to sensible names
jpn_1720 = jpn_1720.rename(columns={'HG': 'HomeGoals', 'AG': 'AwayGoals'})

# Not interested in anything with na in this dataset
jpn_1720 = jpn_1720.dropna(how='all')
...
jpn_params = solve_parameters_decay(jpn_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
jpn_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
HomeTeam = ['CF Montreal', 'New England Revolution', 'New York City', 'Orlando City', 
            'Toronto FC', 'Houston Dynamo', 'Minnesota United', 'St. Louis City', 'Colorado Rapids', 
            'Los Angeles Galaxy', 'San Jose Earthquakes', 'Seattle Sounders', 'Sporting Kansas City']

# List of away teams in the fixtures we are intrested in.
# WARNING this has to be in the same order as above.
AwayTeam = ['Inter Miami', 'Chicago Fire', 'Philadelphia Union', 'Atlanta Utd', 'DC United', 
            'Austin FC', 'Real Salt Lake', 'Vancouver Whitecaps', 'FC Cincinnati', 'Charlotte', 
            'FC Dallas', 'New York Red Bulls', 'Portland Timbers']
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(jpn_params, i, j, max_goals=10)
    ...
    jpn_prediction = jpn_prediction.append(home_away_trans)
