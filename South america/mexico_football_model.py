...
# -*- coding: utf-8 -*-
"""
Created on Thu May 25 16:53:21 2023
...
mex_all = pd.DataFrame()

    
    # Concatenate all of them together into 1 DataFrame
mex_all = pd.read_csv("https://www.football-data.co.uk/new/MEX.csv")

# Ensure that the date is ina  sensible format, day/month/year
mex_all['Date'] = pd.to_datetime(mex_all['Date'],  format='%d/%m/%Y')
...
mex_all['time_diff'] = (max(mex_all['Date']) - mex_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
mex_1720 = mex_all[['Date', 'Home','Away', 'HG','AG', 'Res', 'time_diff']]

# Rename some columns to sensible names
mex_1720 = mex_1720.rename(columns={'HG': 'HomeGoals', 'AG': 'AwayGoals'})

# Not interested in anything with na in this dataset
mex_1720 = mex_1720.dropna(how='all')
...
mex_params = solve_parameters_decay(mex_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
mex_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(mex_params, i, j, max_goals=10)
    ...
    mex_prediction = mex_prediction.append(home_away_trans)
