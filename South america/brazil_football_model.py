...
# -*- coding: utf-8 -*-
"""
Created on Thu May 25 16:51:00 2023
...
bra_all = pd.DataFrame()

    
    # Concatenate all of them together into 1 DataFrame
bra_all = pd.read_csv("https://www.football-data.co.uk/new/BRA.csv")

# Ensure that the date is ina  sensible format, day/month/year
bra_all['Date'] = pd.to_datetime(bra_all['Date'],  format='%d/%m/%Y')
...
bra_all['time_diff'] = (max(bra_all['Date']) - bra_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
bra_1720 = bra_all[['Date', 'Home','Away', 'HG','AG', 'Res', 'time_diff']]

# Rename some columns to sensible names
bra_1720 = bra_1720.rename(columns={'HG': 'HomeGoals', 'AG': 'AwayGoals'})

# Not interested in anything with na in this dataset
bra_1720 = bra_1720.dropna(how='all')
...
bra_params = solve_parameters_decay(bra_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
bra_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
HomeTeam = ['Corinthians', 'Internacional', 'Atletico-MG', 'Bragantino', 'Botafogo RJ']

# List of away teams in the fixtures we are intrested in.
# WARNING this has to be in the same order as above.
AwayTeam = ['Fluminense', 'Bahia', 'Palmeiras', 'Santos', 'America MG']
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(bra_params, i, j, max_goals=10)
    ...
    bra_prediction = bra_prediction.append(home_away_trans)
