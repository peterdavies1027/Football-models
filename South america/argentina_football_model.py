...
# -*- coding: utf-8 -*-
"""
Created on Thu May 25 16:41:24 2023
...
arg_all = pd.read_csv("https://www.football-data.co.uk/new/ARG.csv")

# Ensure that the date is ina  sensible format, day/month/year
arg_all['Date'] = pd.to_datetime(arg_all['Date'],  format='%d/%m/%Y')
...
arg_all['time_diff'] = (max(arg_all['Date']) - arg_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
arg_1720 = arg_all[['Date', 'Home','Away', 'HG','AG', 'Res', 'time_diff']]

# Rename some columns to sensible names
arg_1720 = arg_1720.rename(columns={'HG': 'HomeGoals', 'AG': 'AwayGoals'})

# Not interested in anything with na in this dataset
arg_1720 = arg_1720.dropna(how='all')
...
arg_params = solve_parameters_decay(arg_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
arg_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(arg_params, i, j, max_goals=10)
    ...
    arg_prediction = arg_prediction.append(home_away_trans)
