...
for year in range(18,21):
    ...
    italy2_all = pd.concat((italy2_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/I2.csv".format(year, year+1, sort=True), encoding = 'unicode escape')))

# Ensure that the date is ina  sensible format, day/month/year
italy2_all['Date'] = pd.to_datetime(italy2_all['Date'],  format='%d/%m/%Y')
...
italy2_all['time_diff'] = (max(italy2_all['Date']) - italy2_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
italy2_1720 = italy2_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
italy2_1720 = italy2_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
italy2_1720 = italy2_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
italy2_YC_1720 = italy2_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
italy2_YC_1720 = italy2_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
italy2_YC_1720 = italy2_YC_1720.dropna(how='all')
...
italy2_params = solve_parameters_decay(italy2_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
italy2_params_YC = solve_parameters_decay_YC(italy2_YC_1720, xi = 0.00325)
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(italy2_params, i, j, max_goals=5)
    ...
    matrix_YC = dixon_coles_simulate_match(italy2_params_YC, i, j, max_goals=10)
