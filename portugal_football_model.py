...
portugal_1820 = pd.DataFrame()
for year in range(18,21):
    portugal_1820 = pd.concat((portugal_1820, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/P1.csv".format(year, year+1, sort=True))))
portugal_1820['Date'] = pd.to_datetime(portugal_1820['Date'],  format='%d/%m/%Y')
portugal_1820['time_diff'] = (max(portugal_1820['Date']) - portugal_1820['Date']).dt.days
portugal_1820 = portugal_1820[['HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]
...
HomeTeam = ['Rio Ave', 'Sp Braga', 'Belenenses', 'Benfica', 'Famalicao', 'Nacional', 'Moreirense']
AwayTeam = ['Pacos Ferreira', 'Sp Lisbon', 'Gil Vicente', 'Santa Clara', 'Tondela', 'Guimaraes', 'Porto']
