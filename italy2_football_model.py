...
def calc_means(italy2_param_dict, homeTeam, awayTeam):
    return [np.exp(italy2_param_dict['attack_'+homeTeam] + italy2_param_dict['defence_'+awayTeam] + italy2_param_dict['home_adv']),
            np.exp(italy2_param_dict['defence_'+homeTeam] + italy2_param_dict['attack_'+awayTeam])]
...
def dixon_coles_simulate_match(italy2_params_dict, homeTeam, awayTeam, max_goals=10):
    team_avgs = calc_means(italy2_params_dict, homeTeam, awayTeam)
    team_pred = [[poisson.pmf(i, team_avg) for i in range(0, max_goals+1)] for team_avg in team_avgs]
    output_matrix = np.outer(np.array(team_pred[0]), np.array(team_pred[1]))
    correction_matrix = np.array([[rho_correction(home_goals, away_goals, team_avgs[0],
                                                   team_avgs[1], italy2_params_dict['rho']) for away_goals in range(2)]
...
def solve_parameters_decay(dataset, xi=0.00325, debug = False, init_vals=None, options={'disp': True, 'maxiter':100},
    ...
    def estimate_paramters(italy2_params):
        score_coefs = dict(zip(teams, italy2_params[:n_teams]))
        defend_coefs = dict(zip(teams, italy2_params[n_teams:(2*n_teams)]))
        rho, gamma = italy2_params[-2:]
...
def italy2_match_day(HomeTeam, AwayTeam):
    matrix = dixon_coles_simulate_match(italy2_params, HomeTeam, AwayTeam, max_goals=5)
...
italy2_1820 = pd.DataFrame()
for year in range(18,20):
    italy2_1820 = pd.concat((italy2_1820, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/I2.csv".format(year, year+1, sort=True), encoding = 'unicode escape')))
italy2_1820['Date'] = pd.to_datetime(italy2_1820['Date'],  format='%d/%m/%Y')

italy2_1418 = pd.DataFrame()
for year in range(14, 18):
    italy2_1418 = pd.concat((italy2_1418, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/I2.csv".format(year, year+1, sort=True), encoding = 'unicode escape')))
italy2_1418['Date'] = pd.to_datetime(italy2_1418['Date'],  format='%d/%m/%y')

italy2_1420 = italy2_1418.append(italy2_1820)

italy2_1420['time_diff'] = (max(italy2_1420['Date']) - italy2_1420['Date']).dt.days
italy2_1420 = italy2_1420[['HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]
italy2_1420 = italy2_1420.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})
italy2_1420 = italy2_1420.dropna(how='all')
...
HomeTeam = ['Chievo', 'Cittadella', 'Empoli', 
            'Pescara', 'Pisa', 'Pordenone', 'Spal']
AwayTeam = ['Ascoli', 'Venezia', 'Lecce', 
            'Salernitana', 'Virtus Entella', 'Cosenza', 
            'Cremonese']
