...
for year in range(18,21):
    ...
    netherlands_all = pd.concat((netherlands_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/N1.csv".format(year, year+1, sort=True))))

# Ensure that the date is ina  sensible format, day/month/year
netherlands_all['Date'] = pd.to_datetime(netherlands_all['Date'],  format='%d/%m/%Y')
...
netherlands_all['time_diff'] = (max(netherlands_all['Date']) - netherlands_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
netherlands_1720 = netherlands_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
netherlands_1720 = netherlands_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
netherlands_1720 = netherlands_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
netherlands_YC_1720 = netherlands_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
netherlands_YC_1720 = netherlands_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
netherlands_YC_1720 = netherlands_YC_1720.dropna(how='all')
...
netherlands_params = solve_parameters_decay(netherlands_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
netherlands_params_YC = solve_parameters_decay_YC(netherlands_YC_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
netherlands_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(netherlands_params, i, j, max_goals=5)
    ...
    matrix_YC = dixon_coles_simulate_match(netherlands_params_YC, i, j, max_goals=10)
    ...
    netherlands_prediction = netherlands_prediction.append(home_away_trans)
