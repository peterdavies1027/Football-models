...
for year in range(22,24):
    ...
    SC_champ_all = pd.concat((SC_champ_all, pd.read_csv("https://www.football-data.co.uk/mmz4281/{}{}/SC1.csv".format(year, year+1, sort=True), encoding = 'unicode escape')))

# Ensure that the date is ina  sensible format, day/month/year
SC_champ_all['Date'] = pd.to_datetime(SC_champ_all['Date'],  format='%d/%m/%Y')
...
SC_champ_all['time_diff'] = (max(SC_champ_all['Date']) - SC_champ_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
SC_champ_1720 = SC_champ_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
SC_champ_1720 = SC_champ_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
SC_champ_1720 = SC_champ_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
SC_champ_YC_1720 = SC_champ_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
SC_champ_YC_1720 = SC_champ_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
SC_champ_YC_1720 = SC_champ_YC_1720.dropna(how='all')
...
SC_champ_params = solve_parameters_decay(SC_champ_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
#SC_champ_params_YC = solve_parameters_decay_YC(SC_champ_YC_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
SC_champ_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
HomeTeam = ['Queens Park', 'Cove Rangers', 'Dundee', 'Partick', 'Raith Rvs']

# List of away teams in the fixtures we are intrested in.
# WARNING this has to be in the same order as above.
AwayTeam = ['Hamilton', 'Inverness C', 'Morton', 'Ayr', 'Arbroath']
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(SC_champ_params, i, j, max_goals=10)
    ...
    SC_champ_prediction = SC_champ_prediction.append(home_away_trans)
