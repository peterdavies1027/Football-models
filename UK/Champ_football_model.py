...
champ_1820 = pd.DataFrame()
for year in range(18,21):
    champ_1820 = pd.concat((champ_1820, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/E1.csv".format(year, year+1, sort=True))))
champ_1820['Date'] = pd.to_datetime(champ_1820['Date'],  format='%d/%m/%Y')
champ_1820['time_diff'] = (max(champ_1820['Date']) - champ_1820['Date']).dt.days
champ_1820 = champ_1820[['HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]
champ_1820 = champ_1820.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})
champ_1820 = champ_1820.dropna(how='all')
     
champ_params = solve_parameters_decay(champ_1820, xi = 0.00325)
