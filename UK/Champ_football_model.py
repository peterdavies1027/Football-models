...
for year in range(18,22):
    ...
    champ_all = pd.concat((champ_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/E1.csv".format(year, year+1, sort=True))))

# Ensure that the date is ina  sensible format, day/month/year
champ_all['Date'] = pd.to_datetime(champ_all['Date'],  format='%d/%m/%Y')
...
champ_all['time_diff'] = (max(champ_all['Date']) - champ_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
champ_1720 = champ_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
champ_1720 = champ_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
champ_1720 = champ_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
champ_YC_1720 = champ_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
champ_YC_1720 = champ_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
champ_YC_1720 = champ_YC_1720.dropna(how='all')
...
champ_params = solve_parameters_decay(champ_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
champ_params_YC = solve_parameters_decay_YC(champ_YC_1720, xi = 0.00325)
...
HomeTeam = ['West Brom', 'Preston', 'Birmingham', 'Bournemouth', 'Huddersfield',
            'Hull', 'Luton', 'Peterboro', 'Stoke', 'Swansea', 'Sheffield United', 
            'Derby']

# List of away teams in the fixtures we are intrested in.
# WARNING this has to be in the same order as above.
AwayTeam = ["Nott'm Forest", 'Fulham', 'Blackpool', 'Coventry', 'Middlesbrough', 'Millwall', 
            'Cardiff', 'Barnsley', 'Blackburn', 'Reading', 'Bristol City', 'QPR']
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix_YC = dixon_coles_simulate_match(champ_params_YC, i, j, max_goals=10)
    ...
    champ_prediction = champ_prediction.append(home_away_trans)
