...
for year in range(18,21):
    ...
    conf_all = pd.concat((conf_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/EC.csv".format(year, year+1, sort=True), encoding = 'unicode escape')))

# Ensure that the date is ina  sensible format, day/month/year
conf_all['Date'] = pd.to_datetime(conf_all['Date'],  format='%d/%m/%Y')
...
conf_all['time_diff'] = (max(conf_all['Date']) - conf_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
conf_1720 = conf_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
conf_1720 = conf_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
conf_1720 = conf_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
conf_YC_1720 = conf_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
conf_YC_1720 = conf_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
conf_YC_1720 = conf_YC_1720.dropna(how='all')
...
conf_params = solve_parameters_decay(conf_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
conf_params_YC = solve_parameters_decay_YC(conf_YC_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
conf_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(conf_params, i, j, max_goals=5)
    ...
    matrix_YC = dixon_coles_simulate_match(conf_params_YC, i, j, max_goals=10)
    ...
    conf_prediction = conf_prediction.append(home_away_trans)
