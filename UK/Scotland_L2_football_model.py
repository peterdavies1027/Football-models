...
for year in range(22,24):
    ...
    SC_L2_all = pd.concat((SC_L2_all, pd.read_csv("https://www.football-data.co.uk/mmz4281/{}{}/SC3.csv".format(year, year+1, sort=True), encoding = 'unicode escape')))

# Ensure that the date is ina  sensible format, day/month/year
SC_L2_all['Date'] = pd.to_datetime(SC_L2_all['Date'],  format='%d/%m/%Y')
...
SC_L2_all['time_diff'] = (max(SC_L2_all['Date']) - SC_L2_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
SC_L2_1720 = SC_L2_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
SC_L2_1720 = SC_L2_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
SC_L2_1720 = SC_L2_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
SC_L2_YC_1720 = SC_L2_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
SC_L2_YC_1720 = SC_L2_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
SC_L2_YC_1720 = SC_L2_YC_1720.dropna(how='all')
...
SC_L2_params = solve_parameters_decay(SC_L2_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
#SC_L2_params_YC = solve_parameters_decay_YC(SC_L2_YC_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
SC_L2_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
HomeTeam = ['Annan Athletic', 'Bonnyrigg Rose', 'East Fife', 'Elgin', 'Stenhousemuir']

# List of away teams in the fixtures we are intrested in.
# WARNING this has to be in the same order as above.
AwayTeam = ['Albion Rvs', 'Dumbarton', 'Stranraer', 'Forfar', 'Stirling']
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(SC_L2_params, i, j, max_goals=10)
    ...
    SC_L2_prediction = SC_L2_prediction.append(home_away_trans)
