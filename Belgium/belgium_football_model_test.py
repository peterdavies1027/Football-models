...
for year in range(18,22):
    ...
    bel_all = pd.concat((bel_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/B1.csv".format(year, year+1, sort=True))))

# Ensure that the date is ina  sensible format, day/month/year
bel_all['Date'] = pd.to_datetime(bel_all['Date'],  format='%d/%m/%Y')
...
bel_all['time_diff'] = (max(bel_all['Date']) - bel_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
bel_1720 = bel_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
bel_1720 = bel_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
bel_1720 = bel_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
bel_YC_1720 = bel_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
bel_YC_1720 = bel_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
bel_YC_1720 = bel_YC_1720.dropna(how='all')
...
bel_params = solve_parameters_decay(bel_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
bel_params_YC = solve_parameters_decay_YC(bel_YC_1720, xi = 0.00325)

# Create a DataFrame with all the values we are interested in
bel_prediction = pd.DataFrame(columns = ['HomeTeam', 'AwayTeam', 'Home win', 'Draw', 
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(bel_params, i, j, max_goals=5)
    ...
    matrix_YC = dixon_coles_simulate_match(bel_params_YC, i, j, max_goals=10)
    ...
    bel_prediction = bel_prediction.append(home_away_trans)
