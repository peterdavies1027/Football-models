...
for year in range(18,21):
    ...
    bund_all = pd.concat((bund_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/D1.csv".format(year, year+1, sort=True))))

# Ensure that the date is ina  sensible format, day/month/year
bund_all['Date'] = pd.to_datetime(bund_all['Date'],  format='%d/%m/%Y')
...
bund_all['time_diff'] = (max(bund_all['Date']) - bund_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
# and the time difference
bund_1720 = bund_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
bund_1720 = bund_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
bund_1720 = bund_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
bund_YC_1720 = bund_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
bund_YC_1720 = bund_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
bund_YC_1720 = bund_YC_1720.dropna(how='all')
...
bund_params = solve_parameters_decay(bund_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
bund_params_YC = solve_parameters_decay_YC(bund_YC_1720, xi = 0.00325)
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(bund_params, i, j, max_goals=5)
    ...
    matrix_YC = dixon_coles_simulate_match(bund_params_YC, i, j, max_goals=10)
