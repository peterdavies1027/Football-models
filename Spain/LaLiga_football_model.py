...
for year in range(18,21):
    ...
    spain_all = pd.concat((spain_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/SP1.csv".format(year, year+1, sort=True))))
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(spain_params, i, j, max_goals=5)
    ...
    matrix_YC = dixon_coles_simulate_match(spain_params_YC, i, j, max_goals=10)
