...
for year in range(18,21):
    ...
    spain2_all = pd.concat((spain2_all, pd.read_csv("http://www.football-data.co.uk/mmz4281/{}{}/SP1.csv".format(year, year+1, sort=True))))

# Ensure that the date is ina  sensible format, day/month/year
spain2_all['Date'] = pd.to_datetime(spain2_all['Date'],  format='%d/%m/%Y')
...
spain2_all['time_diff'] = (max(spain2_all['Date']) - spain2_all['Date']).dt.days

######

# We are only interested in the date, home team, away team, goals, results
spain2_1720 = spain2_all[['Date', 'HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]

# Rename some columns to sensible names
spain2_1720 = spain2_1720.rename(columns={'FTHG': 'HomeGoals', 'FTAG': 'AwayGoals'})

# Not interested in anything with na in this dataset
spain2_1720 = spain2_1720.dropna(how='all')

######

# Same dataset as above however we are interested in yellow cards rather than goals
spain2_YC_1720 = spain2_all[['HomeTeam', 'AwayTeam', 'HY', 'AY', 'FTR', 'time_diff']]

# Rename some columns so they are of more use
spain2_YC_1720 = spain2_YC_1720.rename(columns={'HY': 'HomeYellowC', 'AY': 'AwayYellowC'})

# Not interested in anything with na in this dataset
spain2_YC_1720 = spain2_YC_1720.dropna(how='all')
...
spain2_params = solve_parameters_decay(spain2_1720, xi = 0.00325)

# Creates variables for ytellow cards for each team
spain2_params_YC = solve_parameters_decay_YC(spain2_YC_1720, xi = 0.00325)
...
for i, j in zip(HomeTeam, AwayTeam):
    ...
    matrix = dixon_coles_simulate_match(spain2_params, i, j, max_goals=5)
    ...
    matrix_YC = dixon_coles_simulate_match(spain2_params_YC, i, j, max_goals=10)
